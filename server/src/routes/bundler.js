...
const type = obj => Object.prototype.toString.call(obj).split(" ")[1].toLowerCase().slice(0, -1);

router.post("/", async (req, res, next) => {
	const generator = async (query, startingUrl = null, obj = null) => {
		try {
			if (startingUrl) {
				obj = await getJson(req.rootUrl + startingUrl)
					.catch(e => console.log("[bundler] Error while getting", req.rootUrl + startingUrl, ":\n", e));
			}

			if (obj && obj["@self"] && obj["@self"].type && obj["@self"].type === "collection") {
				return {
					...obj,
					collection: await Promise.all(obj.collection
						.map(async item => await generator(query, item.replace(req.rootUrl, ""))))
						.catch(e => console.log("[bundler] Error while populating collection:", e))
				}
			}
			...
			switch (queryType) {
				...
				case "object":
					...
					Object.keys(obj).map(key => {
						...
						if (data && data.length) {
							keyUrl = { ...keyUrl, [data[2]]: "/" + data[1].replace("_", "/") + "/" + obj[key] }
						}
					});
